...
export default function Specialists({ services, reload, t }: Props) {
  ...
  const [hours, setHours] = useState<Array<{ weekday: number; start: string; end: string; active: boolean }>>(
    dayLabels.map((_, weekday) => ({ weekday, start: '09:00', end: '18:00', active: weekday < 5 }))
  );
  ...
  const remove = async (item: Specialist) => {
    if (!window.confirm(t('specialists.deleteConfirm', `Удалить специалиста «${item.name}»?`))) return;
    try {
