(function () {
  ...
  function setStyles() {
    ...
    style.textContent = `
      #bookly-dashboard-v2{display:block!important;width:100%!important;margin:0!important;padding:0!important}
      .bookly-v2-wrap{max-width:680px;margin:0 auto;padding:4px 2px 118px;color:#101318;font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
      ...
      .bookly-v2-business:after{content:"";position:absolute;width:220px;height:220px;border-radius:50%;right:-80px;top:-100px;background:rgba(255,255,255,.08)}
      ...
      .bookly-v2-action{border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.1);color:#fff;border-radius:13px;padding:10px 13px;font-size:12px;font-weight:800;cursor:pointer}
  ...
  function getContext() {
    ...
    return { nav, section: nav?.parentElement || null };
  }

  function isHome(nav) {
    const active = nav?.querySelector('button.active');
    const text = (active?.innerText || '').toLowerCase();
    return text.includes('глав') || text.includes('home');
  }
  ...
  function hideLegacy(section, root) {
    Array.from(section.children).forEach((child) => {
      if (child === root || child.matches(navSelector)) return;
      child.dataset.booklyV2Hidden = '1';
  ...
  function clickExisting(textIncludes) {
  ...
  function render(root, data) {
    ...
    const upcoming = bookings.filter((x) => x.status !== 'cancelled' && (x.day > today || (x.day === today && String(x.start) >= currentTime))).sort((a, b) => `${a.day} ${a.start}`.localeCompare(`${b.day} ${b.start}`));
    ...
    root.innerHTML = `
      <div class="bookly-v2-wrap">
        ...
        <section class="bookly-v2-section">
          ...
          ${todayBookings.length ? todayBookings.slice(0, 6).map((x) => `<div class="bookly-v2-row"><div class="bookly-v2-time">${esc(String(x.start || '').slice(0, 5))}</div><div class="bookly-v2-client"><strong>${esc(x.client_name || 'Клиент')}</strong><span>${esc(x.service_name || 'Запись')} · ${esc(String(x.client_phone || ''))}</span></div><div class="bookly-v2-badge">${x.status === 'confirmed' ? 'Confirmed' : esc(x.status || '')}</div></div>`).join('') : '<div class="bookly-v2-empty">На сегодня записей нет</div>'}
        </section>
        <section class="bookly-v2-section">
        ...
        <section class="bookly-v2-section">
          ...
          ${upcoming.length ? upcoming.slice(0, 4).map((x) => `<div class="bookly-v2-row"><div class="bookly-v2-time">${esc(String(x.start || '').slice(0, 5))}</div><div class="bookly-v2-client"><strong>${esc(x.client_name || 'Клиент')}</strong><span>${esc(x.day)} · ${esc(x.service_name || 'Запись')}</span></div></div>`).join('') : '<div class="bookly-v2-empty">Пока нет будущих записей</div>'}
        </section>
        <section class="bookly-v2-section">
          <div class="bookly-v2-section-head"><h2 class="bookly-v2-section-title">Для клиентов</h2><span class="bookly-v2-section-link">${live ? 'Live' : 'Pro'}</span></div>
    ...
    root.querySelectorAll('[data-v2-action]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        if (action === 'open-client') {
          if (!live) return clickExisting('оплатить подписку');
          tg()?.openTelegramLink ? tg().openTelegramLink(link) : window.open(link, '_blank');
        } else if (action === 'settings') clickExisting('настройки');
        else if (action === 'bookings') clickExisting('записи');
        else if (action === 'new-booking') clickExisting('добавить запись');
        else if (action === 'services') clickExisting('услуги');
        else if (action === 'schedule') clickExisting('график');
        else if (action === 'subscription') {
          clickExisting(live ? 'управление подпиской' : 'оплатить подписку');
          if (!live) setTimeout(() => clickExisting('активировать pro'), 30);
        } else if (action === 'copy-link') {
          navigator.clipboard?.writeText(link).then(() => {
            const strong = button.querySelector('strong');
            if (strong) {
              strong.textContent = 'Скопировано';
              setTimeout(() => { strong.textContent = 'Скопировать'; }, 1400);
            }
          });
  ...
  async function mountDashboard() {
    if (loading) return;
    const { nav, section } = getContext();
    if (!nav || !section) return;

    if (!isHome(nav)) {
      if (mountedSection) showLegacy(mountedSection);
      if (v2Root?.parentNode) v2Root.remove();
      v2Root = null;
      mountedSection = null;
      mountedNav = null;
      loadedKey = '';
    ...
    if (mountedSection !== section || !v2Root || !section.contains(v2Root)) {
      if (v2Root?.parentNode) v2Root.remove();
      mountedSection = section;
      mountedNav = nav;
      v2Root = document.createElement('div');
      v2Root.id = 'bookly-dashboard-v2';
      section.insertBefore(v2Root, section.firstElementChild || nav);
      loadedKey = '';
    }
    ...
    if (loadedKey === currentKey && v2Root.dataset.loaded === '1') return;
    ...
    try {
      ...
      const { nav: latestNav, section: latestSection } = getContext();
      if (!latestNav || latestSection !== mountedSection || !isHome(latestNav) || !v2Root) return;
      render(v2Root, data);
  ...
  function observeNav(nav) {
    if (mountedNav === nav && navObserver) return;
    if (navObserver) navObserver.disconnect();
    mountedNav = nav;
    navObserver = new MutationObserver(() => {
      window.setTimeout(() => { void mountDashboard(); }, 30);
    });
  ...
  function boot() {
    ...
    document.addEventListener('click', (event) => {
      ...
      if (target instanceof Element && target.closest(navSelector)) {
        window.setTimeout(() => { void mountDashboard(); }, 40);
      }
    }, true);
    ...
    window.setTimeout(() => { void mountDashboard(); }, 60);
  }
