...
const telegramMiniApp = Boolean(window.Telegram?.WebApp?.initData);

/* Paddle is website-only. Inside the Telegram Mini App we do not initialize
   Paddle, perform trial checks, or load the Paddle SDK. */
if (telegramMiniApp) {
  (window as any).__booklyPaddleEnvBridge = true;
  (window as any).__booklyPaddleReady = Promise.resolve();
} else {
  ...
  async function getProfileTrialAvailable(): Promise<boolean | null> {
    ...
    if (!api || !initData) return null;

    try {
      const response = await fetch(api + '/admin/subscription/trial-status', {
  ...
  function installProfileTrialPriceGuard(): void {
    if (
      window.Paddle?.__booklyExternalRedirect ||
    ...
    const originalOpen = window.Paddle.Checkout.open.bind(window.Paddle.Checkout);
    const guardedOpen = async (options: any = {}) => {
      const trialAvailable = await getProfileTrialAvailable();

      if (trialAvailable === false && prices.noTrialBase) {
        const items = Array.isArray(options.items)
          ? options.items.map((item: any, index: number) =>
              index === 0 ? {...item, priceId: prices.noTrialBase} : item
            )
          : options.items;

        return originalOpen({...options, items});
      }
  ...
  async function initializePaddle(): Promise<void> {
    ...
    if (window.Paddle.__booklyExternalRedirect) {
      return;
    }
